...
class Hangman
	...
	#  		:push => false				Send live updates of each turn to Firebase as an event, default true
	...
	def initialize(options = {})
		...
		@options[:id] ||= Time.new.strftime("%Y-%m-%d=%H:%M")
	...
	def update_gameState
		...
		response = Firebase.push(@options[:id], @gameState[:turns].last) if @options[:push]
